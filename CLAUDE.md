...
- `mcp.ts` — Registers `vmhq_status` plus three MCP tools per enabled service: `*_api_reference`, `*_operation`, `*_request`. Tool names are derived from `service.id`. Also registers `home_assistant_pinned_entities` when Home Assistant is enabled and `HOME_ASSISTANT_PINNED_ENTITIES` is set, and `adguard_combined_stats` (per-instance `/stats` plus an aggregated total via `combineAdguardStats()`) when both AdGuard Home instances (`adguard` and `adguard2`) are enabled.
